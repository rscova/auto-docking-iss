...
class Env:
    ...
    def compute_reward(self):
        ...
        if abs(self.state[0]) > 0.1:
            reward_y = -(abs(self.state[0]) - 0.1) / 2.9
        else:
            reward_y = -(abs(self.state[0]) - 0.1) / 0.1

        if abs(self.state[1]) > 0.1:
            reward_y = -(abs(self.state[1]) - 0.1) / 2.9
        else:
            reward_y = -(abs(self.state[1]) - 0.1) / 0.1

        if abs(self.state[2]) > 0.1:
            reward_z = -(abs(self.state[2]) - 0.1) / 2.9
        else:
            reward_z = -(abs(self.state[2]) - 0.1) / 0.1
    ...
    def update_done(self):
        ...
        if (abs(self.state[0]) >= self.max_state[0] or abs(self.state[1]) >= self.max_state[1] or abs(self.state[2]) >= self.max_state[2]):
            self.done = 1
            self.done_reward = -200.0
    
        elif abs(self.state[0]) < 0.2 and abs(self.state[1]) < 0.2 and abs(self.state[2]) < 0.2:
            self.done = 1
            self.done_reward = 200.0
